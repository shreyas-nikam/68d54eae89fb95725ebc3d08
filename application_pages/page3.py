...
def run_page3():
    # Ensure that `synthetic_data_preprocessed`, `model`, `X`, `y`, `spd`, `eod` are in session state
    if st.session_state.synthetic_data_preprocessed is None or st.session_state.model is None or st.session_state.X is None or st.session_state.y is None or st.session_state.spd is None or st.session_state.eod is None:
...
""", language="python")
    ...
    st.markdown("""
    The code executes the `reweight` function to mitigate bias in our `synthetic_data`.n
